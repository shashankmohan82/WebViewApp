...
public class AutoCouponsHeadService extends Service {
    ...
    private String sMsg = "";
    private OverlayToSvcReceiver overlayToSvcReceiver;
    private WebviewToSvcReceiver webviewToSvcReceiver;
    ...
    public void onCreate() {
        ...
        overlayToSvcReceiver = new OverlayToSvcReceiver();
        IntentFilter if1 = new IntentFilter(OverlayToSvcReceiver.OVERLAY_TO_SVC);
        LocalBroadcastManager.getInstance(this).registerReceiver(overlayToSvcReceiver, if1);

        webviewToSvcReceiver = new WebviewToSvcReceiver();
        IntentFilter if2 = new IntentFilter(WebviewToSvcReceiver.WEBVIEW_TO_SVC);
        LocalBroadcastManager.getInstance(this).registerReceiver(webviewToSvcReceiver, if2);
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if (inBounded) {
                            ...
                            stopService(new Intent(AutoCouponsHeadService.this, AutoCouponsHeadService.class));
                            inBounded = false;
                            break;
                        }
    ...
    public void onDestroy() {
        ...
        LocalBroadcastManager.getInstance(this).unregisterReceiver(overlayToSvcReceiver);
        LocalBroadcastManager.getInstance(this).unregisterReceiver(webviewToSvcReceiver);
        super.onDestroy();
    }
    ...
    public class WebviewToSvcReceiver extends BroadcastReceiver {
        public static final String WEBVIEW_TO_SVC = "autocoupons.com.webviewapp.webviewtosvc";
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.d("broadcast", "data received from webview");
            if(intent.hasExtra("COUPON_NUM")) {
                OverlayObservable.getInstance().updateValue(intent.getIntExtra("COUPON_NUM", -1));
            }
        }
    }

    public class OverlayToSvcReceiver extends BroadcastReceiver {
        public static final String OVERLAY_TO_SVC = "autocoupons.com.webviewapp.overlaytosvc";
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.d("broadcast", "data received from overlay");
            if(intent.hasExtra("COUPONS"))
                WebviewObservable.getInstance().updateValue(intent.getStringExtra("COUPONS"));
        }
    }
